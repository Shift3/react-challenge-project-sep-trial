...
export const Edit = ({editOrder, order, setIsEdit}) => {
    ...
    const [edittedQuantity, setEdittedQuantity] = useState("1");
    ...
    return(
        <div className="edit-container">
          ...
          <select
            value={edittedQuantity}
            onChange={(event) => setEdittedQuantity(event.target.value)}
          >
          ...
          <button
            ...
            className="order-btn"
            onClick={()=>{
                editOrder(order._id, edittedQuantity, editedItem)
                setIsEdit(false);
            }}
          >
            Confirm Change!
